...
class TestAI:
    def value_function(self,chessboard,role,verbose=False):
        ...
        probs = np.random.random(len(rec_pos))
        ...
        # deepsearch = DeepSearch(verbose=verbose)

        # num = 2
        # if verbose:
        #     print("-- MCTS: Naive Computer with full cost deep-search strategy for role {role}".format(
        #         role=role
        #     ))
        # bestposition = deepsearch.play(chessboardinfo=chessboardinfo,num=num)
        # rec_best_pos = chessboard.pos2rec(bestposition)

        # positions, rec_positions = chessboard.get_availables()
        # action_prob = list()
        # for action in rec_positions:
        #     if action == rec_best_pos:
        #         action_prob.append((action,1))
        #     else:
        #         action_prob.append((action,0)) 
...
class DeepSearch:
    ...
    def play(self,chessboardinfo,num=3):
        self.chessboardinfo = chessboardinfo.copy() # Local version
        point = self.__widesearch(num=num)
        return point

    def __widesearch(self,num):
